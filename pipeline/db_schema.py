...
_TABLE_NOTES = {
    ...
    "sales": [
        ...
        "-- Year: EXTRACT(YEAR FROM NULLIF(sales_date,'')::timestamptz) = EXTRACT(YEAR FROM CURRENT_DATE)",
    ],
    "targets": [
        "-- ⚠ year and month are NUMERIC INTEGERS — NEVER cast to timestamptz!",
        "-- This year: WHERE t.year = EXTRACT(YEAR FROM CURRENT_DATE)",
        "-- Achieved = SUM(sales.grand_total_in_usd) WHERE salesOwner=userId AND status='Confirm' AND year matches",
    ],
    "outreaches": [
